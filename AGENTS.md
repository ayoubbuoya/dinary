...
- The current implementation phase is a **local SQLite app with optional Supabase backup**: Expo SQLite remains the operational source of truth, and Supabase Auth (Google) plus Postgres provide opt-in authenticated backup and multi-device sync. See `docs/decisions/0001-supabase-auth-and-cloud-sync.md`. Sign-in must never be required to use the app, and sync failures must never block local use. Do not add real voice recording/transcription or AI-model execution unless the task explicitly moves the project beyond this phase.
...
- Expo SQLite is the operational source of truth. Supabase is the authenticated backup/multi-device copy only; write locally first and queue for sync, never the reverse. Do not add any other backend, remote database, or network-dependent data flow unless the user explicitly asks.
...
- Cloud sync must not block normal offline use: treat every sync failure as non-fatal and keep pending changes queued.
- Records that sync need tombstones (`deleted_at`), not hard deletes, and every local mutation must mark the row `sync_state = 'pending'`. Never silently discard a conflicting transaction edit; preserve the losing version in `sync_conflicts`.
...
- Run `npm test` (Jest) when touching sync, money, or date logic, and `npx tsc --noEmit` for type safety. Typed routes are generated by the dev server, so a newly added route fails typecheck until `npx expo start` has regenerated `.expo/types`.
